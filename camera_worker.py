...
def result_handler_worker(
    ...
    # ── Appearance cache (histogram-based re-identification) ──────────────
    # When a KCF tracker drops (person leaves frame), their HSV clothing
    # histogram is stored here for HIST_CACHE_TTL seconds.  When a new
    # detection appears with no active tracker, we compare its histogram
    # against the cache to decide: same person returning → skip alert,
    # genuinely new person → fire alert.
    _appearance_cache: list = []
    HIST_CACHE_TTL     = 60.0   # seconds to keep entry after last_seen
    HIST_SIM_THRESHOLD = 0.80   # cv2.HISTCMP_CORREL threshold for "same person"

    def _compute_histogram(frame, bbox):
        """
        Compute a normalized HSV histogram of the clothing region (middle 60%
        vertically) of the detection crop.  Returns a (18×16) float32 array
        or None on failure.
        """
        try:
            ...
            if bh < 10 or bw < 10:
                return None
            pad_y = int(bh * 0.20)          # skip top 20% (head) and bottom 20% (feet)
            crop  = frame[y1 + pad_y : y2 - pad_y, x1:x2]
            if crop.size == 0:
                return None
            hsv  = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
            hist = cv2.calcHist([hsv], [0, 1], None, [18, 16], [0, 180, 0, 256])
            cv2.normalize(hist, hist, 0, 1, cv2.NORM_MINMAX)
            return hist   # shape (18, 16), float32
        except Exception:
            return None

    def _find_cached_person(hist):
        """
        Compare *hist* against the appearance cache.
        Prunes expired entries, then returns the best matching cache entry
        (dict) if similarity >= HIST_SIM_THRESHOLD, otherwise None.
        """
        now = time.time()
        # Prune expired entries in-place
        _appearance_cache[:] = [
            e for e in _appearance_cache
            if now - e["last_seen"] < HIST_CACHE_TTL
        ]
        if hist is None:
            return None
        best_entry = None
        best_score = HIST_SIM_THRESHOLD
        for entry in _appearance_cache:
            try:
                score = cv2.compareHist(hist, entry["histogram"], cv2.HISTCMP_CORREL)
            except Exception:
                continue
            if score > best_score:
                best_score = score
    ...
    while True:
        ...
        for vt in _viol_trackers:
            ...
            if ok:
                ...
                # Keep cache TTL alive while person is still visible
                for ce in _appearance_cache:
                    if ce["session_id"] == vt["session_id"]:
                        ce["last_seen"] = time.time()
                        break
            else:
                vt["fails"] += 1
        ...
        for det in filtered_detections:
            ...
            if is_new_person and bbox:
                # ── Histogram re-ID: check appearance cache ───────────────
                hist   = _compute_histogram(resized, bbox)
                cached = _find_cached_person(hist)

                if cached is not None and vk in cached["violation_classes"]:
                    # Same person re-entered frame, same violation already alerted.
                    # Refresh their cache TTL and re-init a KCF tracker so they
                    # are tracked again while visible — but do NOT fire a new alert.
                    ...
                    if new_t is not None:
                        _viol_trackers.append({
                            "tracker":    new_t,
                            "bbox":       list(bbox),
                            "alerted":    set(cached["violation_classes"]),
                            "fails":      0,
                            "session_id": cached["session_id"],
                        })
                    logged_this_frame.add(vk)
                    continue   # skip record_violation — already alerted for this person

                # Either a brand-new person or same person with a new violation class
                session_id     = cached["session_id"] if cached else str(uuid.uuid4())
                is_truly_new   = cached is None
            else:
                hist         = None
                cached       = None
            ...
            if record_violation(det, allow_snapshot=is_truly_new, session_tracker_id=session_id, skip_cooldown=True):
                ...
                if bbox:
                    if existing is not None:
                        ...
                        # Add or update the appearance cache entry
                        if hist is not None:
                            if cached is not None:
                                # Same person, new violation class — update cache
                                cached["violation_classes"].add(vk)
                                cached["last_seen"] = time.time()
                                cached["histogram"] = hist   # refresh appearance
                            else:
                                # Brand new person — create cache entry
                                _appearance_cache.append({
                                    "session_id":        session_id,
                                    "histogram":         hist,
                                    "violation_classes": {vk},
                                    "last_seen":         time.time(),
                                })
