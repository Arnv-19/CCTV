      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-80 shadow-2xl">
        ...
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <input ref={nameRef}
              className="w-full bg-slate-800 text-white rounded-lg px-3 py-2 text-sm border border-slate-600 outline-none focus:border-blue-500"
          ...
          <div>
            ...
            <input type="color"
              className="h-9 w-full rounded-lg cursor-pointer border border-slate-600 bg-slate-800"
              value={color} onChange={e => setColor(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">Priority (0–100)</label>
          ...
          <div className="flex gap-3 pt-1">
            <button type="submit"
              className="flex-1 py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors">
              Save
            </button>
            <button type="button"
  ...
  return (
    <div className="flex h-full bg-slate-950">
      ...
      <div className="flex-1 relative flex flex-col" ref={containerRef} onMouseMove={handleMouseMove}>
        {/* Mini toolbar */}
        <div className="flex items-center gap-3 px-4 py-2 bg-slate-900/80 border-b border-slate-800 text-xs">
          <span className="text-slate-400 font-mono">{cameraId}</span>
          {roi.mode === 'draw' && (
            <>
          ...
          {roi.mode === 'edit' && <span className="text-blue-400">● Editing</span>}
        ...
        <div className="flex-1 relative">
          ...
          {/* Hint bar */}
          {roi.mode === 'draw' && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/70 text-slate-300 text-xs px-3 py-1.5 rounded-full pointer-events-none">
      ...
      {/* Side panel */}
      <div className="w-68 border-l border-slate-800 flex-shrink-0" style={{ width: 272 }}>
        <ROIPanel
          rois={roi.rois}
          selectedROIId={roi.selectedROIId}
          mode={roi.mode}
